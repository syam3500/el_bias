...
simulated_RRF <- sample(RRF_norm, size = N_extractables, replace = FALSE)
## alpha) No Correction
...
simulated_RRF <- sample(RRF_norm, size = N_extractables, replace = TRUE)
## beta) UF corrections
...
ggplot(plot_data, aes(x = Method, y = Rate*100, fill = Type)) +
  ...
  labs(
    ...
    subtitle = "Conc. LogNormal distribution, RRF normal dist. N(0, 0.4²)",
    y = "Rate (%)",
    x = "Method") +
